...
export default function PolaroidCard({ trip, onClick }: Props) {
  ...
  return (
    <div className="relative w-52 h-72 cursor-pointer group flex-shrink-0" onClick={onClick}>
      {/* Bottom stacked card */}
      <div
      ...
      {/* Top card */}
      <div
        className="absolute inset-0 bg-polaroid rounded-sm shadow-lg p-2.5 transition-all duration-300 group-hover:-translate-y-2.5"
        style={{ transform: "rotate(0.8deg)" }}
      >
        {/* Photo area */}
        <div className="relative w-full h-40 overflow-hidden bg-muted/30 rounded-[2px]">
        ...
        <div className="mt-2.5 px-0.5">
          <p className="font-georgia font-bold text-[11px] text-ink truncate">{trip.name}</p>
          {trip.destination && (
            <p className="text-[9px] text-muted-foreground truncate mt-0.5">{trip.destination}</p>
          )}
