...
export default function TripItinerary({ tripId, tripStartDate, tripEndDate, isOwner }: Props) {
  ...
  const addItem = async (dayNumber: number) => {
    ...
    try {
      const dayDate = tripStartDate 
        ? format(new Date(new Date(tripStartDate).getTime() + (dayNumber - 1) * 24 * 60 * 60 * 1000), 'yyyy-MM-dd')
        : null;

      const maxOrder = Math.max(0, ...items.filter(i => i.day_number === dayNumber).map(i => i.display_order));
    ...
    <div className="relative max-w-3xl mx-auto px-4">
      ...
      {days.length === 0 && !addingDay && (
        <div className="text-center py-12">
          ...
          {isOwner && (
            <Button onClick={() => setAddingDay(1)} className="gap-2 bg-amber hover:bg-amber/90">
              <Plus size={18} weight="bold" />
              Add First Day
            </Button>
          )}
        ...
        return (
          <div key={day} className="mb-8">
            ...
            <div className="ml-8 border-l-2 border-dotted border-amber/30 pl-8 space-y-6">
              ...
              {isOwner && addingDay === day && (
                <div className="relative">
                  <div className="absolute -left-[52px] top-0">
                    <PirateX size={28} className="opacity-50" />
                  </div>

                  <div className="bg-card border-2 border-dashed border-amber/40 rounded-lg p-4 space-y-3">
      ...
      {/* Add New Day Form */}
      {isOwner && addingDay && addingDay > maxDay && (
        <div className="mb-8">
          <div className="flex items-center gap-3 mb-4">
            <Calendar size={24} weight="duotone" className="text-amber" />
            <h3 className="font-georgia text-xl font-bold text-ink">
          ...
          <div className="ml-8 border-l-2 border-dotted border-amber/30 pl-8">
            <div className="relative">
              <div className="absolute -left-[52px] top-0">
                <PirateX size={28} className="opacity-50" />
              </div>
